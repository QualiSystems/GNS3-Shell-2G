...
class TemplateInstanceDetails(BaseInstanceDetails):
    ...
    @property
    def should_connect_mgmt(self):
        return self._deploy_attribs.get("{}.Connect To Management Network".format(
            self._deployment_path), "").lower() == "true"

    @property
    def should_shrink_interfaces(self):
        return self._deploy_attribs.get("{}.Shrink Network Adapters".format(
            self._deployment_path), "").lower() == "true"
